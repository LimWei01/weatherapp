...
import { saveLocationToFirestore, getLocationsFromFirestore } from './firestoreUtils';
...
export const synchronizeStorage = async (userId) => {
  try {
    ...
    const firestoreLocations = await getLocationsFromFirestore(userId);
...
export const saveLocationToStorage = async (location, originalName = null) => {
  try {
    ...
    if (existingLocationIndex === -1) {
      ...
      if (user) {
        try {
          await saveLocationToFirestore(user.uid, location, originalName);
